...
\par
\textbf{Schritt 5} besteht aus dem Zusammenstellen des Befehls zur Ausführung des MapReduce-Jobs (siehe Listing \ref{lst:ncdc concatenate start script}). Der vollständig dokumentierte Befehl ist in Appendix \ref{appendix lst:ncdc concatenate start script} zu finden. 
\begin{itemize}
    \item Zeile 1 weist Hadoop an, ein JAR auszuführen. Es wird der Class Path zum Hadoop Streaming JAR angegeben, welches mit dem Framework mitgeliefert wird.
    ...
    \item Zeile 9 gibt die Mapper-Klasse an. Würde die Java API statt der Streaming API verwendet, stünde hier eine Java Klasse. Hier wird der Alias aus Zeile 3 angegeben.
\end{itemize}
